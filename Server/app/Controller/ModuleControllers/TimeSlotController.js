...
TimeSlotController = function() {
     ...
     * @return {RESPONSE}
     */
    this.getMyTimeSlots = function(UserInstance, res) {
        //get all time slots
        Lecturer.findOne({
            where: {
                status: 1,
    ...
    this.getAvailableTimeSlots = function(DataInstance, res) {
        TimeSlot.findAll({
                where: {
                    ...
                    id: {
                        $notIn: [Sequelize.literal("SELECT a.TimeSlotId FROM appointment a WHERE a.appointmentDate = '" + Helper.JSDateToSQLDate(DataInstance.date) + "' AND a.cancel = 0")],
                    }
                }
            })
            .then(function(data) {
